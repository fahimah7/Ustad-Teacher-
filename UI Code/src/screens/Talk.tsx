...
import { askTeacher, historyOnThisPage, pageLabel, plainText, useTutorStatus } from "../lib/tutor";
...
export function Talk() {
  ...
  const page = book ? school.pages[book.id] ?? lesson!.pdf.start : 1;

  const [speechLang, setSpeechLang] = useState<Lang>(lang === "en" ? "en" : "fa");
  const opening = useMemo(() => {
    const last = [...(school.chats[bookId] ?? [])].reverse().find((x) => x.from === "ustad" && x.reply.kind === "teacher" && !x.reply.failed);
    if (last && last.from === "ustad" && last.reply.kind === "teacher") { const t = plainText(last.reply.text); return { en: t, fa: t }; }
    return { en: "Ask me about this page.", fa: "دربارهٔ این صفحه از من بپرسید." };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [id]);
  const [text, setText] = useState(opening);
  const words = useMemo(() => tokenizeWords(speechLang === "en" ? text.en : text.fa), [text, speechLang]);
  ...
  const say = useCallback((t = text, r = rate) => {
    stopAll();
    const plain = speechLang === "en" ? t.en : t.fa;
  ...
  const answer = async (q: string) => {
    if (!book) return;
    if (tutor.state !== "ready") {
      const wait = { en: "I'm still getting ready. Please ask again in a moment.", fa: "هنوز آماده می‌شوم. لطفاً کمی بعد دوباره بپرسید." };
      setText(wait); say(wait); setTyped(""); return;
    }
    setPhase("thinking");
    setTyped(null);
    const msgs = school.chats[bookId] ?? [];
    const lastAsked = [...msgs].reverse().find((x) => x.from === "me" || x.from === "page");
    const moved = !!lastAsked && lastAsked.page !== page;
    const add: ChatMsg[] = [...(moved ? [{ id: uid(), from: "page" as const, page }] : []), { id: uid(), from: "me", text: q, page }];
    update((s) => ({ ...s, chats: { ...s.chats, [bookId]: [...(s.chats[bookId] ?? []), ...add] } }));
    const r = await askTeacher({ bookId, page, question: q, history: moved ? [] : historyOnThisPage(msgs), lang: speechLang, name });
    update((s) => ({ ...s, chats: { ...s.chats, [bookId]: [...(s.chats[bookId] ?? []), { id: uid(), from: "ustad", reply: r, page }] } }));
    const spoken = plainText(r.text);
    const next = { en: spoken, fa: spoken };
    setText(next);
    say(next);
  };
  ...
  const src = book ? `${m({ en: "Grade", fa: "صنف" })} ${book.grade} · ${pageLabel(bookId, page, lang)}` : null;
  const status = phase === "speaking" ? m({ en: "Ustad is speaking", fa: "استاد صحبت می‌کند" })
    ...
    : tutor.state !== "ready" ? m({ en: "Ustad is getting ready…", fa: "استاد آماده می‌شود…" })
    : m({ en: "Hold the mic to talk, or type", fa: "برای گفتن، میکروفون را نگه دارید یا بنویسید" });
  ...
  return (
    <Full bg="#1C1433">
      <div className="safe-top" style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", maxWidth: 560, width: "100%", margin: "0 auto" }}>
        ...
        {src && <div style={{ marginTop: 8 }}><SourceChip>{src}</SourceChip></div>}
