...
export function Me() {
  ...
  const rows: { icon: IconName; bg: string; c: string; title: string; sub: string; to: string }[] = [
    ...
    { icon: "ask", bg: "#FFE3EA", c: "#9A1240", title: m({ en: "Your questions", fa: "سوال‌های شما" }), sub: m({ en: `${school.questions.length} saved on this device`, fa: `${num(school.questions.length)} در این دستگاه` }), to: "/ask" },
    { icon: "library", bg: "#DDF6F3", c: "#005F5B", title: m({ en: "Change grade or book", fa: "تغییر صنف یا کتاب" }), sub: m({ en: `Grade ${grade} now`, fa: `اکنون صنف ${num(grade)}` }), to: "/welcome/grade" },
  ];
      ...
      <div className="screen">
        <div className="col" style={{ background: "#7443F0", padding: "max(var(--safe-top), 28px) 0 70px", position: "relative", overflow: "hidden" }}>
          ...
          <div className="desk-pad u-enter" style={{ padding: "22px 20px 0", position: "relative" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
              <div style={{ height: 30, padding: "0 12px", borderRadius: 999, background: "rgba(255,255,255,.18)", display: "flex", alignItems: "center", font: "800 12px/1 var(--font-latin)", color: "#FFFFFF" }}>{m({ en: "Grade", fa: "صنف" })} {num(grade)}</div>
              {name && <div style={{ height: 30, padding: "0 12px", borderRadius: 999, background: "rgba(255,255,255,.18)", display: "flex", alignItems: "center", font: "800 12px/1 var(--font-latin)", color: "#FFFFFF" }}>{name}</div>}
        ...
        <div className="col desk-pad" style={{ margin: "-44px 0 0", padding: "0 16px 24px", position: "relative", display: "flex", flexDirection: "column", gap: 10, maxWidth: 720 }}>
          ...
          <div className="u-enter" style={{ ["--i" as string]: 6, background: "#E3F7F4", borderRadius: 22, padding: 14, display: "flex", gap: 12, alignItems: "flex-start" } as React.CSSProperties}>
            ...
            <div style={{ font: "600 14px/1.5 var(--font-latin)", color: "#005F5B" }}>{m({ en: "Everything here stays on this device, locked. No account, and your name is never saved. The teacher runs on this computer: the app never connects to the internet.", fa: "همه چیز در همین دستگاه و قفل‌شده می‌ماند. حسابی نیست و نام شما هرگز ذخیره نمی‌شود. معلم روی همین کمپیوتر کار می‌کند: برنامه هرگز به انترنت وصل نمی‌شود." })}</div>
          </div>
