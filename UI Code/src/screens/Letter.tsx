...
import { LETTER_PROMPT, LETTER_THEMES } from "../content/rights";
...
export function LetterSent() {
  ...
  const nodes: [string, string, string][] = [
    ...
    [m({ en: "Someone you trust", fa: "کسی که به او اعتماد دارید" }), m({ en: "carries the file to a connected computer", fa: "فایل را به کمپیوتری که انترنت دارد می‌برد" }), "#D81E57"],
    [m({ en: "Readers", fa: "خوانندگان" }), m({ en: "it can be posted for others to read", fa: "می‌شود آن را برای خواندن دیگران نشر کرد" }), "#FFB31A"],
  ];
  const exportIt = () => {
    if (!l?.outbox) return;
    ...
    <Full bg="#1C1433">
      <div className="safe-top col" style={{ maxWidth: 560, minHeight: "100dvh", display: "flex", flexDirection: "column" }}>
        <div className="u-enter" style={{ padding: "28px 24px 0" }}>
          ...
          <div style={{ font: "600 15px/1.5 var(--font-latin)", color: "#BDB6D9", marginTop: 8 }}>{m({ en: "It is sealed with a one-time key, so it is not linked to your phone or your other letters. Names and places were taken out, but your own words can still say who you are: read it once more before you hand it on.", fa: "این نامه با یک کلید یک‌بارمصرف مهر شده و به گوشی یا نامه‌های دیگر شما ربط ندارد. نام‌ها و جاها حذف شده‌اند، ولی کلمات خودتان هنوز می‌توانند شما را بشناسانند: پیش از سپردن، یک بار دیگر آن را بخوانید." })}</div>
        </div>
