...
import { useLang, type Lang, SEP } from "../lib/i18n";
import { speak } from "../lib/speech";
import { suggestionsFor, stripMd } from "../lib/tutor";
...
export function AskHub() {
  ...
  const here = school.continueAt.lessonId;
  const sugg = suggestionsFor(here).length ? suggestionsFor(here) : suggestionsFor(school.reading);

  const start = (text: string, lessonId = here) => {
    if (!text.trim()) return;
    update((s) => ({
      ...s, reading: lessonId,
      chats: { ...s.chats, [lessonId]: [...(s.chats[lessonId] ?? [])] },
    }));
        ...
        <div className="col" style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ background: "#D81E57", paddingBottom: 70, position: "relative", overflow: "hidden", paddingTop: "max(var(--safe-top), 28px)" }}>
            ...
            <div className="desk-pad u-enter" style={{ padding: "22px 20px 0", position: "relative" }}>
              ...
              <div style={{ font: "600 15px/1.45 var(--font-latin)", color: "#FFE3EA", marginTop: 8, maxWidth: 300 }}>{m({ en: "Any question, by voice or text. Answers start from your lessons.", fa: "هر سوالی، با صدا یا متن. جواب‌ها از درس‌های تو شروع می‌شوند." })}</div>
            </div>
          </div>
          ...
          <div className="desk-pad grid-2" style={{ gap: 8, padding: "0 16px" }}>
            {sugg.map((s, i) => (
              <Press key={i} ledge={4} className="u-enter" onClick={() => start(m(s.q), s.lessonId)} style={{ ["--i" as string]: 2 + i, height: 52, borderRadius: 16, background: "#FFFFFF", border: "2px solid #EFE6DA", boxShadow: "0 4px 0 #EFE6DA", display: "flex", alignItems: "center", gap: 10, padding: "0 12px", textAlign: "start" } as React.CSSProperties}>
          ...
          <div className="desk-pad" style={{ padding: "20px 20px 10px", display: "flex", alignItems: "baseline" }}>
            ...
            <div style={{ marginInlineStart: "auto", font: "700 13px/1 var(--font-latin)", color: "#6F6A88" }}>{m({ en: "saved on this phone", fa: "در همین گوشی ذخیره شده" })}</div>
          </div>
            ...
            {school.questions.slice(0, 8).map((sq, i) => {
              ...
              return (
                <div key={sq.id} className="u-fade" style={{ ["--i" as string]: 5 + i, display: "flex", gap: 12, alignItems: "center", background: "#FFFFFF", borderRadius: 16, padding: "8px 12px 8px 8px" } as React.CSSProperties}>
                  <button className="press flat" onClick={() => start(sq.q, sq.lessonId)} style={{ display: "flex", gap: 12, alignItems: "center", flex: 1, minWidth: 0, textAlign: "start" }}>
                    <Thumb subject={l.subject} grade={l.grade} w={36} h={44} r="10px 10px 5px 5px" />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ font: "700 15px/1.25 var(--font-latin)", color: "#1C1433" }}>{sq.q}</div>
                      <div style={{ font: "600 12px/1.5 var(--font-latin)", color: "#6F6A88" }}>{m(sj.short)} {l.grade}{SEP}{m({ en: "Lesson", fa: "درس" })} {l.unit.n}.{l.lesson.n}</div>
                    </div>
                  </button>
                  <Press flat aria-label={m({ en: "Listen", fa: "بشنو" })} onClick={() => speak(stripMd(sq.answer ? m(sq.answer) : sq.q), { lang })} style={{ width: 44, height: 44, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 22 }}>
