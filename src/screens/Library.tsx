...
import { bookById, booksOf, gradeState, PLACED_GRADE } from "../content/library";
...
function Shelf() {
  ...
  const [grade, setGrade] = useState(PLACED_GRADE);
  const books = booksOf(grade).filter((b) => b.installed);
  const installedGrades = Array.from({ length: 12 }, (_, i) => i + 1).filter((g) => gradeState(g) !== "missing").length;
      ...
      <div style={{ padding: "12px 16px 0", display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <div style={{ marginInlineStart: "auto", height: 30, padding: "0 10px", borderRadius: 999, background: "#DDF6F3", display: "flex", alignItems: "center", gap: 5, font: "800 12px/1 var(--font-latin)", color: "#005F5B" }}>
            <Icon n="ondevice" s={15} c="#005F5B" w={2.4} />{num(installedGrades)} {m({ en: "grades on this phone", fa: "صنف در این گوشی" })}
          </div>
        </div>
          ...
          if (st === "partial" && !on) {
            return (
              <Press key={g} ledge={4} className="u-pop" onClick={() => setGrade(g)} style={{ ...base, background: "linear-gradient(0deg,#FFFFFF 50%,#FFF8EF 50%)", border: "3px solid #FFFFFF", boxShadow: "0 4px 0 #E6DCCD", gap: 1, font: "900 17px/1 var(--font-latin)", color: "#1C1433" }}>
                {num(g)}<span style={{ font: "800 9px/1 var(--font-latin)", color: "#9A5B00" }}>{num(booksOf(g).filter((b) => b.installed).length)} {m({ en: "book", fa: "کتاب" })}</span>
              </Press>
            );
          }
          return (
  ...
  return (
    <div className="col">
      <div style={{ background: s.bg, padding: "max(var(--safe-top), 12px) 0 28px", position: "relative", overflow: "hidden" }}>
        ...
        <div className="desk-pad" style={{ display: "flex", gap: 16, alignItems: "flex-end", padding: "14px 20px 0" }}>
          ...
          <div className="u-enter" style={{ minWidth: 0 }}>
            ...
            <h1 style={{ margin: "8px 0 0", font: "900 30px/1.05 var(--font-latin)", color: s.fg }}>{s.name.en}</h1>
      ...
      <div className="desk-pad" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
        {b.units.map((u, ui) => {
          const done = (school.chapters[b.subject]?.done ?? 0) >= u.n && b.grade === PLACED_GRADE;
          return (
            <div key={u.n} className="u-enter" style={{ ["--i" as string]: ui, background: "#FFFFFF", borderRadius: 22, boxShadow: "0 4px 0 #EFE6DA", padding: 14 } as React.CSSProperties}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1, font: "800 17px/1.2 var(--font-latin)", color: "#1C1433" }}>{m(u.title)}</div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {u.lessons.map((l) => {
                  const full = isFull(l);
                  return (
                    <Press key={l.id} flat onClick={() => go(`/lesson/${l.id}`)} style={{ minHeight: 48, borderRadius: 14, background: full ? "#FFF8EF" : "transparent", border: full ? "0" : "2px dashed #EFE6DA", display: "flex", alignItems: "center", gap: 10, padding: "0 12px", textAlign: "start" }}>
                      <div style={{ font: "800 13px/1 var(--font-latin)", color: "#6F6A88", width: 30 }}>{num(u.n)}.{num(l.lesson.n)}</div>
                      <div style={{ flex: 1, font: `${full ? 700 : 600} 15px/1.25 var(--font-latin)`, color: full ? "#1C1433" : "#6F6A88" }}>{full ? m(l.lesson.title) : m({ en: `p. ${l.source.pages[0]}–${l.source.pages[1]}`, fa: `ص ${num(l.source.pages[0])}–${num(l.source.pages[1])}` })}</div>
