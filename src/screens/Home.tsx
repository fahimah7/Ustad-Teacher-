...
import { lessonById, PLACED_GRADE } from "../content/library";
...
function useContinue() {
  const { school } = useApp();
  const c = school.continueAt;
  const l = lessonById(c.lessonId)!;
  return { c, l, s: SUBJECTS[l.subject] };
}
    ...
    <div className="col">
      ...
      <div style={{ background: "#00827E", paddingBottom: 60, position: "relative", overflow: "hidden", paddingTop: "max(var(--safe-top), 12px)" }}>
        ...
        <div style={{ height: 56, display: "flex", alignItems: "center", padding: "0 12px 0 6px", gap: 4, position: "relative" }}>
          ...
          <div style={{ font: "700 16px/1 var(--font-latin)", color: "#DDF6F3" }} className={lang !== "en" ? "fa" : undefined}>{greeting(lang)}</div>
      ...
      <div className="u-enter-far" style={{ ["--i" as string]: 1, margin: "-46px 16px 0", background: "#FFFFFF", borderRadius: 28, boxShadow: "0 6px 0 #E6DCCD", padding: 16, position: "relative", display: "flex", flexDirection: "column", gap: 14 } as React.CSSProperties}>
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          <div style={{ width: 74, height: 86, borderRadius: "37px 37px 12px 12px", overflow: "hidden", flex: "none" }}><Art seed="cont-math-ch3" w={160} h={160} cols={2} bg="#1C1433" anim /></div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "inline-flex", height: 24, padding: "0 10px", borderRadius: 999, background: "#DDF6F3", color: "#005F5B", font: "800 12px/24px var(--font-latin)" }}>{m({ en: "Grade", fa: "صنف" })} {num(l.grade)}{SEP}{m(s.name)}</div>
            <div style={{ font: "800 21px/1.2 var(--font-latin)", color: "#1C1433", marginTop: 6 }}>{m({ en: "Chapter", fa: "فصل" })} {num(l.unit.n)}{SEP}{m(l.unit.title)}</div>
            <div style={{ font: "600 13px/1.4 var(--font-latin)", color: "#6F6A88", marginTop: 2 }}>{m({ en: "Page", fa: "صفحه" })} {num(c.page)}{SEP}{m({ en: `about ${c.minutesLeft} min left`, fa: `حدود ${num(c.minutesLeft)} دقیقه مانده` })}</div>
          </div>
        </div>
        ...
        <Btn tone="saf" iconEnd="arrow" onClick={() => go(`/lesson/${c.lessonId}`)}>{t("continueLabel")}</Btn>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "baseline", padding: "20px 20px 10px" }}>
        ...
        <button className="press flat" onClick={() => go("/library")} style={{ marginInlineStart: "auto", font: "700 14px/1 var(--font-latin)", color: "#00827E" }}>{m({ en: "Grade", fa: "صنف" })} {num(PLACED_GRADE)}{SEP}{m({ en: "All", fa: "همه" })}</button>
      </div>
      <div style={{ display: "flex", gap: 10, padding: "0 16px" }}>
        {tiles.map((k, i) => {
          const sj = SUBJECTS[k];
          const ch = school.chapters[k];
          return (
            <Press key={k} className="u-tile" onClick={() => go(`/library?book=g${PLACED_GRADE}-${k}`)} style={{ ["--i" as string]: 5 + i, flex: 1, minWidth: 0, height: 96, borderRadius: "20px 20px 10px 10px", background: sj.bg, boxShadow: `0 5px 0 ${sj.ledge}`, padding: 10, display: "flex", flexDirection: "column", justifyContent: "space-between", textAlign: "start" } as React.CSSProperties}>
              <Icon n={sj.icon} s={24} c={sj.fg} w={2.4} />
              <div>
                <div style={{ font: "800 13px/1.1 var(--font-latin)", color: sj.fg }}>{m(sj.short)}</div>
                <div style={{ font: "700 11px/1.4 var(--font-latin)", color: sj.sub }}>{num(ch?.done ?? 0)} {m({ en: "of", fa: "از" })} {num(ch?.total ?? 0)}</div>
              </div>
            </Press>
          );
        })}
...
function QuickFive({ i, big }: { i: number; big?: boolean }) {
  ...
  const { m } = useLang();
  return (
    <Press ledge={big ? 6 : 5} className="u-enter" onClick={() => go("/quiz/math8-u3")} style={{ ["--i" as string]: i, margin: big ? 0 : "16px 16px 0", width: big ? "100%" : "calc(100% - 32px)", height: big ? 92 : 72, borderRadius: big ? 26 : 22, background: "#1C1433", boxShadow: `0 ${big ? 6 : 5}px 0 #0A0714`, display: "flex", alignItems: "center", gap: big ? 16 : 12, padding: big ? "0 16px 0 20px" : "0 12px 0 16px", textAlign: "start", flex: "none" } as React.CSSProperties}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ font: `600 ${big ? 13 : 12}px/1.4 var(--font-latin)`, color: "#BDB6D9" }}>{m({ en: "Fractions · about 3 minutes", fa: "کسرها · حدود ۳ دقیقه" })}</div>
      </div>
      ...
      <div style={{ height: 290, background: "#00827E", position: "relative", overflow: "hidden", flex: "none", padding: "44px 44px 0" }}>
        ...
        <div style={{ position: "relative", display: "flex", alignItems: "flex-start", gap: 32 }}>
          <div style={{ flex: 1 }} className="u-enter">
            <div style={{ font: "700 17px/1 var(--font-latin)", color: "#DDF6F3" }}>{m({ en: `Grade ${PLACED_GRADE} · ${day} ${part}`, fa: `صنف ${num(PLACED_GRADE)}` })}</div>
      ...
      <div style={{ flex: 1, display: "flex", gap: 24, padding: "0 44px 32px", marginTop: -92, position: "relative", minHeight: 0 }}>
        <div style={{ flex: 1.7, display: "flex", flexDirection: "column", gap: 22, minWidth: 0 }}>
          <div className="u-enter-far" style={{ background: "#FFFFFF", borderRadius: 32, boxShadow: "0 7px 0 #E6DCCD", padding: 24, display: "flex", gap: 26, alignItems: "center" }}>
            <div style={{ width: 150, height: 184, borderRadius: "75px 75px 20px 20px", overflow: "hidden", flex: "none" }}><Art seed="cont-math-ch3" w={160} h={160} cols={2} bg="#1C1433" anim /></div>
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12, minWidth: 0 }}>
              <div style={{ display: "inline-flex", alignSelf: "flex-start", height: 28, padding: "0 12px", borderRadius: 999, background: "#DDF6F3", color: "#005F5B", font: "800 13px/28px var(--font-latin)" }}>{m({ en: "Grade", fa: "صنف" })} {num(l.grade)}{SEP}{m(s.name)}</div>
              <div style={{ font: "900 32px/1.1 var(--font-latin)", letterSpacing: "-.02em", color: "#1C1433" }}>{m({ en: "Chapter", fa: "فصل" })} {num(l.unit.n)}{SEP}{m(l.unit.title)}</div>
              <div style={{ font: "600 15px/1.4 var(--font-latin)", color: "#6F6A88" }}>{m({ en: `Page ${c.page} · about ${c.minutesLeft} minutes left · 5 practice questions waiting`, fa: `صفحه ${num(c.page)} · حدود ${num(c.minutesLeft)} دقیقه مانده · ۵ سوال تمرینی منتظر است` })}</div>
              ...
              <div style={{ display: "flex", gap: 12, marginTop: 2, flexWrap: "wrap" }}>
                <Btn tone="saf" iconEnd="arrow" style={{ padding: "0 32px" }} onClick={() => go(`/lesson/${c.lessonId}`)}>{t("continueLabel")}</Btn>
                <Btn tone="outline" icon="ask" fg="#D81E57" fs={17} style={{ padding: "0 24px" }} onClick={() => update((x) => ({ ...x, reading: c.lessonId }))}>{m({ en: "Ask about this chapter", fa: "درباره این فصل بپرس" })}</Btn>
              </div>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "baseline" }}>
            <h2 style={{ margin: 0, font: "800 22px/1 var(--font-latin)", color: "#1C1433" }}>{m({ en: "Your subjects", fa: "مضمون‌های تو" })}</h2>
            <button className="press flat" onClick={() => go("/library")} style={{ marginInlineStart: "auto", font: "700 15px/1 var(--font-latin)", color: "#00827E" }}>{m({ en: "All 9 books", fa: "همه ۹ کتاب" })}</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 14 }}>
            {shelf.map((k, i) => (
              <Press key={k} ledge={0} flat className="u-tile" onClick={() => go(`/library?book=g${PLACED_GRADE}-${k}`)} style={{ ["--i" as string]: 2 + i, display: "block" } as React.CSSProperties}>
                <Cover subject={k} grade={PLACED_GRADE} badge={34} radius="18px 18px 8px 8px" fs={13} />
              </Press>
            ))}
          </div>
        </div>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16, minWidth: 360 }}>
          <div className="u-enter" style={{ ["--i" as string]: 2, flex: 1, minHeight: 0, background: "#FFFFFF", borderRadius: 32, boxShadow: "0 7px 0 #E6DCCD", overflow: "hidden", display: "flex", flexDirection: "column" } as React.CSSProperties}>
            <Chat lessonId={school.reading} variant="panel" />
          </div>
          <QuickFive i={4} big />
        </div>
      </div>
