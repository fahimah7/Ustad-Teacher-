...
import { CHAPTERS_IN, setById } from "../content/practice";
...
export function ChapterDone() {
  ...
  const key = set ? `${set.subject}${set.grade}` : "";
  const unitKey = set ? `unit:${key}-u${set.unit}` : "";
  ...
  useEffect(() => {
    ...
    update((st) => st.lessonsDone.includes(unitKey) ? st : {
      ...
      chapterLights: { ...st.chapterLights, [key]: Math.min(CHAPTERS_IN[key] ?? 8, (st.chapterLights[key] ?? 0) + 1) },
    });
  }, [set, key, unitKey, update]);

  if (!set) return null;
  const total = CHAPTERS_IN[key] ?? 8;
    ...
    <Full bg="#1C1433">
      <div className="safe-top" style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", maxWidth: 520, margin: "0 auto", width: "100%" }}>
        ...
        <div style={{ alignSelf: "stretch", padding: "0 16px calc(22px + var(--safe-bottom))", display: "flex", flexDirection: "column", gap: 10 }}>
          <Btn tone="saf" iconEnd="arrow" onClick={() => go("/library", { replace: true })}>{m({ en: "Next chapter", fa: "فصل بعدی" })}</Btn>
